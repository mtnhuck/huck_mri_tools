function [sysmeans systemlist ]=corrmat2systemmeansabs(zmat)
%Change path here to include your system labels of choice
    ...
    for j=1:length(systemlist)
        for k=1:size(zmat,3)
            if i==j
                ...
                sysmeans(i,j,k)=nanmean(abs(tmp(ind)));
            else
                sysmeans(i,j,k)=nanmean(nanmean(abs(zmat(find(systems05==systemlist(i)),find(systems05==systemlist(j)),k))));
            end
        end
    end
end
